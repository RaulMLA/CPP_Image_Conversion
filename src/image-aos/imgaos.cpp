...
// Función MAIN para el programa AOS.
...
// call_oper_aos, que se encargan de preparar la llamada a la función que realizará la
...
int main(int argc, char * argv[])
{
  ...
  for (const auto & file : directory_iterator(in_path)) {
    ...
    if (oper == "copy") call_copy_aos(in_path, out_path, file);
    if (oper == "histo") call_hist_aos(in_path, out_path, file);
    if (oper == "mono") call_mono_aos(in_path, out_path, file);
    if (oper == "gauss") call_gauss_aos(in_path, out_path, file);
